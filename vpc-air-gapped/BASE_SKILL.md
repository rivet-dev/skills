...
Before starting any work, check if the user's project is on the latest version of RivetKit (latest: 2.3.3-rc.2). Look at the `rivetkit` version in the user's `package.json` (check both `dependencies` and `devDependencies`). If the installed version is older than 2.3.3-rc.2, inform the user and suggest upgrading:
...
1. Install RivetKit (latest: 2.3.3-rc.2)
    ```bash
    npm install rivetkit@2.3.3-rc.2
    ```
