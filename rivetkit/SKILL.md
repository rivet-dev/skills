...
Before starting any work, check if the user's project is on the latest version of RivetKit (latest: 2.3.6). Look at the `rivetkit` version in the user's `package.json` (check both `dependencies` and `devDependencies`). If the installed version is older than 2.3.6, inform the user and suggest upgrading:

```bash
npm install rivetkit@2.3.6
```
...
1. Install RivetKit (latest: 2.3.6)
    ```bash
    npm install rivetkit@2.3.6
    ```
